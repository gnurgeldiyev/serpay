...
const { validateData } = require('../helpers/poet')
...
exports.getAll = (req, res) => {
    ...
    if (q && q === 'true') {
        return Poet.find({ is_deleted: true })
    ...
    return Poet.find({ is_deleted: false })
        .sort({ firstname: 1, created_at: 1 })
        .then((poets) => {
            if (!poets.length) {
                return res.status(404).json({
...
exports.delete = (req, res) => {
    ...
    Poet.findByIdAndUpdate(
        id,
        {
            $set: { is_deleted: query !== 'false' },
        },
        { new: true }
    )
        .then((poet) => {
