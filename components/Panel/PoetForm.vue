  ...
  <div>
    ...
    >
      <el-form ref="form" :model="form" :rules="rules" class="form">
        ...
        <el-form-item prop="bio" label="Short Bio">
          <el-input
            v-model="form.bio"
            :rows="4"
            :disabled="type === 'view'"
        ...
        <el-form-item prop="death_date" label="Death Date">
          <el-input
        ...
        <el-form-item prop="wiki_link" label="Wikipedia Link">
          <el-input
            v-model="form.wiki_link"
            :disabled="type === 'view'"
            :placeholder="
              type === 'view' ? '' : 'Wikipedia page link of the poet'
            "
          />
        </el-form-item>
        <el-form-item prop="avatar" label="Avatar">
          ...
          >
            ...
            <i v-else class="el-icon-plus upload-icon" />
          </el-upload>
        </el-form-item>
      ...
      rules: {
        fullname: [
          ...
          },
          {
            min: 1,
            max: 100,
            message: "Length should be max 100 characters",
            trigger: "blur",
          },
        ],
        birth_date: [],
        ...
        avatar: [
          {
            required: true,
            message: "Please upload a avatar",
            trigger: "blur",
          },
        ],
      },
  ...
  methods: {
    ...
    close() {
      ...
      if (this.fileList.length > 0) {
        this.$axios
          .$delete(`/api/poets/upload/${this.fileList[0]}`)
          .then(() => {
            this.fileList = [];
          })
          .catch((err) => {
    ...
    onRemove(file, fileList) {
      this.$axios
        .$delete(`/api/poets/upload/${file.response.file.filename}`)
        .then(() => {
